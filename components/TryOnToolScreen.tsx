    ...
    return (
      <View className="flex-1 bg-black">
        ...
        <SafeAreaView className="flex-1 justify-between px-6 pb-8">
            ...
            {/* ScrollView en el resultado también por seguridad */}
  ...
  return (
    <View className="flex-1 bg-[#0f0f0f]">
      ...
      <SafeAreaView className="flex-1">
        <View className="flex-row justify-between items-center px-6 mb-2 z-10">
        ...
        >
            ...
            <Text className="text-zinc-300 text-lg mb-6">{subtitle}</Text>

            <View className="gap-4">
            {/* CORRECCIÓN CRÍTICA: 
                En lugar de `flex-1` (que encoge los cuadros en pantallas pequeñas),
                usamos `h-[280px]` (altura fija) para garantizar que siempre sean grandes.
                El ScrollView manejará el overflow.
            */}
            <TouchableOpacity 
                onPress={() => openPicker('user')}
                activeOpacity={0.9}
                className={`h-[280px] rounded-3xl border-2 border-dashed relative overflow-hidden bg-black/40 backdrop-blur-xl ${userImage ? 'border-indigo-500' : 'border-white/20'}`}
            >
                {userImage ? (
                <>
            ...
            {/* TARJETA 2: PRENDA - Altura Fija */}
            <TouchableOpacity 
                onPress={() => openPicker('garment')}
                activeOpacity={0.9}
                className={`h-[280px] rounded-3xl border-2 border-dashed relative overflow-hidden bg-black/40 backdrop-blur-xl ${garmentImage ? 'border-purple-500' : 'border-white/20'}`}
            >
                {garmentImage ? (
                <>
